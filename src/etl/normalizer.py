...
def adjust_for_inflation(
    ...
    if isinstance(df, pl.DataFrame):
        ...
        factor_items = [
            (int(year), float(base_ipc / ipc)) for year, ipc in IPC_DATA.items()
        ]
        ...
        df = df.with_columns(
            [
                (pl.col(value_column) * pl.col("FACTOR").fill_null(1.0)).alias(
                    "VALOR_AJUSTADO"
                )
            ]
...
def create_temporal_features(
    df: pl.DataFrame | pd.DataFrame, date_column: str = "FECHA_RADICA_TEXTO"
) -> pl.DataFrame | pd.DataFrame:
    """
    ...
    if isinstance(df, pl.DataFrame):
        ...
        date_columns_to_process = [
            col
            for col in ["FECHA_RADICA_TEXTO", "FECHA_APERTURA_TEXTO"]
        ...
        for col in date_columns_to_process:
            ...
            if df[col].dtype in [pl.Date, pl.Datetime]:
                ...
                date_expr = pl.coalesce(
                    [
                        ...
                        pl.col(col).str.strptime(
                            pl.Date, format="%Y-%m-%d %H:%M:%S", strict=False
                        ),
                        pl.col(col).str.strptime(
            ...
            if failed > 0:
                print(
                    f"    ⚠ {failed:,} de {total:,} fechas no parseadas ({failed / total * 100:.2f}%)"
                )
            else:
